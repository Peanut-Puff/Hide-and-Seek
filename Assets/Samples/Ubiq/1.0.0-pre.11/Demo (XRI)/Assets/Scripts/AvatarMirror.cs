    ...
    private void Update()
    {
        ...
        for (int ai = 0; ai < avatarManager.transform.childCount; ai++)
        {
            ...
            for (int i = 0; i < renderers.Count; i++)
            {
                ...
                {
                    case Plane.XY:
                        if (Mathf.Abs(localPos.x) > mirrorSize.x * 30.5f ||
                            Mathf.Abs(localPos.y) > mirrorSize.y * 30.5f)
                        {
                            outOfBounds = true;
                        }
                        break;
                    case Plane.YZ:
                        if (Mathf.Abs(localPos.y) > mirrorSize.y * 30.5f ||
                            Mathf.Abs(localPos.z) > mirrorSize.z * 30.5f)
                        {
                            outOfBounds = true;
                        }
                        break;
